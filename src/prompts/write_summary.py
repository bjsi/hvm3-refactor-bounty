...
def optimize_for(program, task_lm, prompt_lm):
    with dspy.context(lm=task_lm):
        tp = dspy.teleprompt.MIPROv2(metric=judge_quality, auto="light", prompt_model=prompt_lm, task_model=task_lm, max_bootstrapped_demos=0, max_labeled_demos=0, num_threads=20)
...
def write_summary():
    ...
    with dspy.context(lm=get_lm("gemini/gemini-1.5-flash-8b")):
